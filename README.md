<!--
**MusicMakerOwO/MusicMakerOwO** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
**Projects** :
- [Fox Box Insurance](https://notfbi.dev/), full discord server backups and data preservation, all open source
- [Easy Invite Tracker](https://github.com/MusicMakerOwO/EasyInviteTracker), minimal discord invite logger
- [Dungeon Crawler](https://github.com/MusicMakerOwO/Buildathon-2025), text rpg game for the Discord Buildathon 2025
- [Self Host Manager](https://github.com/MusicMakerOwO/SelfHostManager), easy process manager for self hosting projects

**Tools** :
- [Micro Base](https://github.com/MusicMakerOwO/MicroBase), premade discord bot to skip project setup

**Contact** :
- Discord: @musicmaker
- Fox Tech Industries: support@notfbi.dev

**About** :

I'm very passionate about online privacy and user experiences. In my work I prioritize code quality and readability, with a particular focus on efficient data structures and algorithms. Currently tech stack is JavaScript, TypeScript, SQL, HTML/CSS, and learning Rust.
